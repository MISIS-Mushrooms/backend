...
class UserBank:
    ...
    def get_user_id_by_name(self, birth_date: dt.date, first_name: str, middle_name: str, last_name: str):
        item = self._df[(self._df['дата рождения'] == birth_date) & (self._df['first'] == first_name) & (self._df['middle'] == middle_name) & (self._df['last'] == last_name)]
        if len(item) == 1:
...
_RE_TIMETABLE = re.compile(r'(?:[cс] (?P<date_from>\d{2}\.\d{2}\.\d{4}) по (?P<date_to>\d{2}\.\d{2}\.\d{4}), )?(?P<dow>[А-Яа-я., ]+) (?P<time_from>\d{2}:\d{2})-(?P<time_to>\d{2}:\d{2})')
...
class GroupBank:
    def __init__(self, groups_path):
        ...
        df['all_timetable'] = df.apply(lambda x: '; '.join([y for y in x[['расписание в активных периодах', 'расписание в закрытых периодах', 'расписание в плановом периоде']] if not pd.isna(y)]), axis=1)
        ...
        for _, itm in dict_df.iterrows():
            threecat_map[(itm['level1'], itm['level2'], itm['leve3'])] = threecat_simple[itm['Разметка: Для ума/ Для души / Для тела'].strip()]
        ...
        group_by_cols = ['направление 1', 'направление 2', 'направление 3', 'округ площадки', 'район площадки', 'is_online']
...
class AttendanceCollator():
    def __call__(self, batch):
        ...
        if 'group_target' in batch[0]:
            ...
            dct['group_sequence_out'] = torch.stack([F.pad(x['group_sequence_out'], (0, max_group_sequence_ln_out - len(x['group_sequence_out']))) for x in batch])
        return dct
...
class UserVectorizer(nn.Module):
    def __init__(self, bank: UserBank):
        ...
        self._born_mort_emb = MLP(in_channels=13, hidden_channels=[64, 128, EMBED_DIM], activation_layer=nn.GELU, dropout=0.1)

    def forward(self, data):
        gender_emb = self._gender_emb(data['user_gender'])
        age_emb = self._age_emb(data['user_age_bin'])
        born_mort_emb = self._born_mort_emb(data['user_born_mort']) + self._born_mort_bias.repeat(data['user_born_mort'].shape[0], 1)
...
class AllVectorizer(nn.Module):
    ...
    def forward(self, data):
        ...
        group_mask = torch.cat([torch.ones([group_seq.shape[0], 1], dtype=data['group_mask'].dtype, device=data['group_mask'].device), data['group_mask']], dim=1)
        ...
        if 'group_sequence_out' in data:
            ...
            return self._loss(all_group_vec, x[group_mask == 1])[group_mask.sum(dim=1).cumsum(0) - 1]
